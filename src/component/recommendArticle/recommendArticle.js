...
export default function RecommendArticle({ setRecommendSize }) {
    ...
    useEffect(() => {
        ...
        setRecommendSize instanceof Function &&
            setRecommendSize(articles.length);
    }, [articles]);
    useEffect(() => {
        (async () => {
        ...
        <>
            {articles.length > 0 && (
                <>
                    ...
                    <div
                        className={`waterFall ${style.waterFall}`}
                        ref={waterFallDOM}
                    >
